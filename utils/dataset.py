...
class CharacterDataset(TensorDataset):
    def __init__(self, data_root, reference_count: int = 1):
        ...
        self.template_list = sorted(glob.glob('{}/*.png'.format(self.template_root)))
        self.character_map = {os.path.basename(path): i for i, path in enumerate(self.template_list)}

        self.script_list = []
        self.writer_list = sorted(glob.glob('{}/*'.format(self.script_root)))
        for writer in tqdm(self.writer_list, desc='loading dataset'):
            character_list = sorted(glob.glob('{}/*.png'.format(writer)))
