...
class SynthesisGenerator(nn.Module):
    ...
    """
    def __init__(self, reference_count: int = 1, writer_count: int = None):
        super(SynthesisGenerator, self).__init__()
        self.structure_encoder = StructureEncoder()
        self.style_encoder = StyleEncoder(reference_count, writer_count)
        self.result_decoder = DenormalizationGenerator()

    def forward(self, reference, template):
        template   = self.structure_encoder(template)
        reference = self.style_encoder(reference, return_logits=False)
        result = self.result_decoder(template, reference)

        return result, template, reference

    @torch.no_grad()
    def inference(self, reference, template, alpha=0.2):
        step = 1/alpha

        style = self.style_encoder(reference)*alpha + self.style_encoder(template.repeat(1, reference.shape[1], 1, 1), return_logits=True)*(1-alpha)
        result = []
        for _ in range(int(step)):
            template = self.structure(template)
            draft = self.result_decoder(template, style)
            result.append(draft)
            template = draft
        return result
    ...
    def style(self, input, return_logits=False):
        output = self.style_encoder(input, return_logits)
        return output
