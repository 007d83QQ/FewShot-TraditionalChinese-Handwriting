...
class StyleEncoder(nn.Module):
    def __init__(self, reference_count: int, writer_count: int):
        ...
        if writer_count is not None:
            self.writer_head = nn.Linear(512, writer_count)
        else:
            self.writer_head = None

    def forward(self, input, return_logits=False):
        feat = self.backbone(input)     # (B, 512)
        if self.writer_head is None or not return_logits:
            return feat
        logits = self.writer_head(feat) # (B, N_writer)
        return feat, logits
